...
export class LikeService extends Common {
  constructor() {
    super();
  }
  ...
  getLikes = async (templateId: number) => {
    try {
      const response = await this.axiosWithAuth.get<ITemplateLike[]>(
        queryConfig.CRUD_LIKES + "/" + templateId
      );
      return response.data;
    } catch (error) {
      const e = error as AxiosError;
  ...
  getLike = async (templateId: number, userId?: number) => {
    try {
      const response = await this.axiosWithAuth.get<ITemplateLike>(
        queryConfig.CRUD_LIKES + "/" + templateId + "/" + userId
      );
      return response.data;
    } catch (error) {
      const e = error as AxiosError;
